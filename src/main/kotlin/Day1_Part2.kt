class Day1_Part2 {
    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            ...
            val ints = input.split(",").map { it.trim() }.map { it.toInt() }.toCollection(ArrayList())
            var index = 0
            val next = generateSequence {
                if (index >= ints.size) index = 0
                ints[index++]
            }.iterator()

            println(find(HashSet(), next, 0))
        }

        private tailrec fun find(seen: HashSet<Int>, next: Iterator<Int>, current: Int): Int {
            return if (seen.add(current))
                find(seen, next, current + next.next())
            else
                current
        }
    }
