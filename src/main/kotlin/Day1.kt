...
fun recursiveWay() {
    java.util.Scanner(input).use { scanner ->
        scanner.useDelimiter("[ ]?,[ ]?")
        println(calculate(0, scanner))
    }
}

tailrec fun calculate(current: Int, scanner: java.util.Scanner): Int =
    if (!scanner.hasNext())
        current
    else
        calculate(current + scanner.nextInt(), scanner)
